...
export default function DashboardClient({ accessToken }: { accessToken: string }) {
  const [kpis,      setKpis]    = useState<FitterKPIs>(DEFAULT_KPIS)
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API}/api/fitter/portal/kpis`, { headers }),
      fetch(`${API}/api/fitter/portal/jobs`, { headers }),
    ])
      .then(async ([kpiRes, jobsRes]) => {
